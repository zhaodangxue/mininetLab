...
#这个脚本是用来测试spine-leaf网络的在同一节点的udp性能，链路带宽为1Gbps，按iperf配置运行udp发包10s,发包速率为1000M(1Gbps），以两种形式发送，一种是h1发送tcp包给自身的device ip 10.1.1.100，另一种是h1发送tcp包给自身的127.0.0.1
def log(s, col="green"):
...
def main():
    ...
    filename = "data/udp_send_h1self.log"
    ...
    result = h1.cmd('iperf -u -c 127.0.0.1 -b 1000M -t 10')
    info(result)
    with open(filename, "a") as f:
        f.write(result+"\n")
    info("*** Running iperf client on h1  ->10.1.1.100\n")
    result = h1.cmd('iperf -u -c  10.1.1.100  -b 1000M -t 10')
    info(result)
    with open(filename, "a") as f:
        f.write(result+"\n")
    net.stop()
